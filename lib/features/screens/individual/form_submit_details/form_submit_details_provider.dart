...
class FormSubmitDetailsProvider extends BaseProvider{
  ...
  Future<void> fetchCompanyData() async {
    try {
      final response = await authRepository.individualCompanyData();
      if (response.isSuccess == true) {
  ...
  Future<void> fetchAgentData() async {
    try {
      final response = await authRepository.individualAgentData(selectedCompanyIds);
      if (response.isSuccess == true) {
        final List rawList = response.data['data'] ?? [];
        agentData = rawList
        ...
        print('adfadsfds=> ${response.data}');
      }
    } catch (error) {
      print('adskjfbaddsakjdsaf=> $error');
    }
  }
  ...
  Future<void> productFormApply(BuildContext context, Product? product) async {
    try {
      ...
      body['agent_id'] = selectedAgentIds;
      ...
      print("BODY => $body");
      final response = await authRepository.individualSubmitProduct(body);
